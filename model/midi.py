...
def parse_midi(path):
    ...
    for message in midi:
        ...
        if message.type == 'control_change' and message.control == 64 and (message.value >= 64) != sustain:
            ...
            event = dict(index=len(events), time=time,
                         type=event_type, note=None, velocity=0)
            events.append(event)

        if 'note' in message.type:
            # MIDI offsets can be either 'note_off' events or 'note_on' with zero velocity
            velocity = message.velocity if message.type == 'note_on' else 0
            event = dict(index=len(events), time=time, type='note',
                         note=message.note, velocity=velocity, sustain=sustain)
            events.append(event)
    ...
    for i, onset in enumerate(events):
        ...
        note = (onset['time'], offset['time'],
                onset['note'], onset['velocity'])
        notes.append(note)

    return np.array(notes)


def save_midi(path, pitches, intervals, velocities, debug="off"):
    """
...
if __name__ == '__main__':
    ...
    def process(input_file, output_file):
        ...
        np.savetxt(output_file, midi_data, '%.6f', '\t',
    ...
    def files():
        for input_file in tqdm(sys.argv[1:]):
            if input_file.endswith('.mid'):
                ...
                print('ignoring non-MIDI file %s' %
